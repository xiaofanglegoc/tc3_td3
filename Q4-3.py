...
class RequestHandler(generic.RequestHandler):
  ...
  #
  # On envoie un document avec la liste des lignes en json
  #
  def send_trains(self):

    conn = sqlite3.connect('sncf.sqlite')
    c = conn.cursor()
    c.execute("SELECT DISTINCT c.hexadecimal, p.code_ligne, p.nom_ligne, p.type_ligne " +\
    ...
    body = json.dumps([{'code':c, 'nom':n, 'couleur': h, 'type': t} for (h,c,n,t) in r])
    self.send(body,headers)


  #
  # On renvoie une réponse json avec le nom de l'image
  #
  def send_image(self):
    headers = [('Content-Type','application/json')];
    body = json.dumps({'img': self.imagename, 'title': ' '.join(self.path_info)})
    self.send(body,headers)
